...
var Trending = class {
  ...
  async renderTrendingBlock(el, config) {
    ...
    try {
      ...
      el.empty();
      const hasAnySimklConversion = items.some(
        (item) => item._zoroMeta?.source === "simkl"
      );
    ...
    const entry = isSearch ? {
      ...
      _zoroMeta: data?._zoroMeta || {
        source: (
          ...
          data?._zoroMeta?.source || this.apiHelper.validateAndReturnSource(config?.source) || this.apiHelper.detectFromDataStructure({ media }) || this.apiHelper.getFallbackSource()
        ),
        mediaType: (() => {
          if (config?.mediaType) return config.mediaType;
