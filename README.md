...
Use code blocks to embed your AniList data:
...
- `username`: AniList username (optional if default is set in settings)
- `listType`: Type of list to display
  - `CURRENT` - Currently watching
  - `COMPLETED` - Completed anime
  - `PLANNING` - Plan to watch
  - `DROPPED` - Dropped anime
  - `PAUSED` - Paused anime
- `showImages`: Show cover images (true/false)
- `showRatings`: Show your personal ratings (true/false)
- `showProgress`: Show watch progress (true/false)
...
```markdown
Check out ![[anilist:username/anime/123456]] - it's amazing!
```

#### Show User Stats

```markdown
My anime stats: ![[anilist:username/stats]]
```
...
The card layout displays your anime/manga in a responsive grid with cover images, titles, progress, ratings, and genre tags.

### Table Layout

The table layout shows your data in a compact table format - perfect for lists with many entries.

### Inline Display

Use inline syntax to reference specific anime/manga or display user statistics within your text.
